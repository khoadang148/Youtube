...
const handleSubmit = (e) => {
  ...
  setInputHistory(prevHistory => {
    ...
    return [input, ...newHistory].slice(0, 25) 
  })
}
  ...
  <div className='flex mr-20'>
    <form onSubmit={handleSubmit} onFocus={handleFocus} onBlur={handleBlur} className="self-center m-[0.1rem] mr-7 flex border-[#222222] pr-4 rounded-full border-[1.2px] text-white  bg-[#222222]  text-base w-[643px] h-12 relative p-[1px]">
            <input
              ...
              className="ml-[1px] pl-4 rounded-l-full w-[643px] border-y-[1px] border-r-[1px] border-l-0 my-[1px] border-[#121212] font-medium p-[0.3rem] text-base mr-14 text-white bg-[#121212] focus:outline-none"
            />
            <button type="submit" className="top-3 right-1 absolute px-[1.25rem] bg-[#222222] text-white border-none focus:border-none">
            <AiOutlineSearch size={22}/>
            </button>
    </form>
            <button type="submit" className="text-center px-[15px] bg-[#222222] text-white border-none focus:border-none rounded-full">
              <img src={require('../assets/mic.png')} alt='' className='w-[23px] h-[22px]'/>
            </button>
  </div>
  <div className='absolute right-0 mr-10 flex justify-around items-center'>
    <MdNotifications size={28} className='mx-[10px]'/>
        ...
        <div className="absolute bg-[#222222] p-2 top-[75px] left-[540px] w-[571px] border-0 rounded-[10px] shadow">
          <ul className="list-none p-0 m-0">
            {inputHistory.map((item, index) => (
              <div className='flex'>
                <Image className='w-[26px] h-[26px] text-white mt-[9px]' src={require('../assets/history.png')} alt='haha'></Image>
              <li key={index} className="text-base cursor-pointer hover:bg-[#272727] p-2 text-white" onClick={() => setInput(item)}>
                {item}
              </li>
              </div>
            ))}
